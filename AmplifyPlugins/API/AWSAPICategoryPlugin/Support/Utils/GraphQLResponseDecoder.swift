...
struct GraphQLResponseDecoder {
    ...
    static func decode<R: Decodable>(graphQLServiceResponse: AWSAppSyncGraphQLResponse,
        ...
        case (.none, .some(let errors)):
            ...
            return GraphQLResponse<R>.failure(.error(<#T##[GraphQLError<[String : JSONValue]>]#>))
    ...
    private static func decodeErrors<R: Decodable>(graphQLErrors: [JSONValue],
                                                   into responseType: R.Type) throws -> [GraphQLError<R>] {
        var responseErrors = [GraphQLError<R>]()
    ...
    private static func decode<R: Decodable>(graphQLError: JSONValue,
        ...
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = ModelDateFormatting.decodingStrategy
        return try decoder.decode(GraphQLError<R>.self, from: serializedJSON)
    }
